"""Experiment #19 (R19) — Finer blend-weight grid around R18's adopted region (w=25%/50%).

Rules (locked before running; see Evervault
research/finance/backtests/results/r19-voltarget-blend-fine-grid.md for full spec).
...
weights w in {10,20,30,40,60}% (fills gaps around R18's coarse 0/25/50/75/100% grid, w=25/50
not re-run here -- already published in R18), monthly rebalance back to target weight, with
a blend-level rebalancing cost (SPY-leg-only, 5bps/side) applied to the weight drift between
sleeves each month, identical methodology to R18. Sleeve A's own internal trading costs are
already embedded in its monthly returns.

Pure stdlib, reads the existing cached data/cache/closes.csv (same cache as R1/R3/R5/R10/
R11/R16/R18) -- no new fetch, no installs.

Run: cd ~/Documents/trading && python3 backtest_sleeve_blend_r19.py
"""
import csv
import os
import statistics
...
BLEND_WEIGHTS = [0.10, 0.20, 0.30, 0.40, 0.60]
# R18 published values, reused for comparison (not recomputed here)
R18_PUBLISHED = {
    0.00: dict(cagr=0.1501, vol=0.1569, sharpe=0.97, sortino=1.44, maxdd=-0.2393, final=3.78),
    0.25: dict(cagr=0.1436, vol=0.1423, sharpe=1.02, sortino=1.47, maxdd=-0.1904, final=3.58),
    0.50: dict(cagr=0.1365, vol=0.1305, sharpe=1.05, sortino=1.50, maxdd=-0.1690, final=3.37),
    0.75: dict(cagr=0.1288, vol=0.1224, sharpe=1.06, sortino=1.51, maxdd=-0.1559, final=3.16),
    1.00: dict(cagr=0.1206, vol=0.1186, sharpe=1.02, sortino=1.47, maxdd=-0.1427, final=2.95),
}


def load_closes():
    if not os.path.exists(CACHE):
        raise SystemExit(f"Cache missing at {CACHE} — expected R1/R3/R5/R10/R18 cache to "
...
def main():
    ...
    print(f"Sanity check — Sleeve B (SPY B&H): CAGR {m_b['cagr']:.2%} "
          ...
          f"(R18 published SPY: CAGR 15.01%, Sortino 1.44, maxDD -23.93%)")
    ...
    print("\nDominance check vs R18's adopted w=25%/w=50% (published, reused not recomputed):")
    for label, pub in (("w=25% (R18)", R18_PUBLISHED[0.25]), ("w=50% (R18)", R18_PUBLISHED[0.50])):
        for w, m, yr in rows:
            beats_sharpe = m["sharpe"] > pub["sharpe"]
            beats_sortino = m["sortino"] > pub["sortino"]
            beats_maxdd = m["maxdd"] > pub["maxdd"]
            clears_cagr = abs(m["cagr"] - m_b["cagr"]) <= 0.015
            dominates = beats_sharpe and beats_sortino and beats_maxdd and clears_cagr
            print(f"  w={w:.0%} vs {label}: Sharpe={'Y' if beats_sharpe else 'n'} "
                  f"Sortino={'Y' if beats_sortino else 'n'} maxDD={'Y' if beats_maxdd else 'n'} "
                  f"CAGRbar={'Y' if clears_cagr else 'n'}  DOMINATES={'YES' if dominates else 'no'}")
