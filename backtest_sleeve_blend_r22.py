"""Experiment R22 — Finer blend-weight grid around R20's w=75% (top-2 sleeve + SPY).

Rules (locked before running; see Evervault
research/finance/backtests/results/r22-top2-blend-fine-grid-around-75.md for full spec).
...
Blended at fixed weights w in {60,65,70,75,80,85,90}%, monthly rebalance back to
target weight, with a blend-level rebalancing cost (SPY-leg-only, 5bps/side) applied
to the weight drift between sleeves each month, identical methodology to R18/R20.
Sleeve A's own internal trading costs are already embedded in its monthly returns.
...
Run: cd ~/Documents/trading && python3 backtest_sleeve_blend_r22.py
"""
import csv
import os
import statistics
...
BLEND_WEIGHTS = [0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90]
