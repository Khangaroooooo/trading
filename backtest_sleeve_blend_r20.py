"""Experiment R20 — Fixed-weight blend: R8 top-2 concentration sleeve + SPY B&H.

Rules (locked before running; see Evervault
research/finance/backtests/results/r20-top2-spy-blend.md for full spec).

Sleeve A = R8 exact spec (11 SPDR sectors, top-2 abs-momentum vs BIL, monthly rebal,
5bps/side, no vol-target overlay). Sleeve B = SPY B&H, same monthly grid. Blended at
fixed weights w in {0, 25, 50, 75, 100}%, monthly rebalance back to target weight,
with a blend-level rebalancing cost (SPY-leg-only, 5bps/side) applied to the weight
drift between sleeves each month, identical methodology to R18
(backtest_sleeve_blend_r18.py). Sleeve A's own internal trading costs are already
embedded in its monthly returns.

Pure stdlib, reads the existing cached data/cache/closes.csv (same cache as
R1/R3/R5/R8/R10/R11/R16/R18) -- no new fetch, no installs.

Run: cd ~/Documents/trading && python3 backtest_sleeve_blend_r20.py
"""
import csv
import os
import statistics
